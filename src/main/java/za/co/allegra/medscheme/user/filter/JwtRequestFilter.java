...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ApplicationUser applicationUser = (ApplicationUser) jwtUserDetailsService.loadUserByUsername(username);
            applicationUser.addAuthority((String) jwtTokenUtil.getAllClaimsFromToken(jwtToken).get("status"));
            if (jwtTokenUtil.validateToken(jwtToken, applicationUser)) {
